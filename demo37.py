...
# Files    
most_popular = pickle.load(open('top50n.pkl','rb'))
pivot_table  = pickle.load(open('pivot_tablen.pkl','rb'))
books        = pickle.load(open('booksn.pkl','rb'))
sc           = pickle.load(open('scn.pkl','rb'))
df_final     = pickle.load(open('df_final.pkl','rb'))

book_name    = most_popular ['Book-Title'].tolist()
author       = most_popular['Book-Author'].tolist()
ratings      = most_popular['Book-Rating'].tolist()
image        = most_popular['Image-URL-M'].tolist()
avg_ratings  = most_popular['avg_rating'].tolist()
...
def recommend_book(book):
    ...
    similar_items = sorted(list(enumerate(sc[index])),key=lambda x:x[1],reverse=True)[1:6]
